...
def zip_(*structures, **kwargs):
  ...
  Args:
    ...
    **kwargs: other args:
        "flatten": Whether to flatten the resulting structure into a tuple.
        Keys of dictionaries will be discarded.

  Returns:
    Nested structure.
  """
  # Named keyword arguments are not allowed after *args in Python 2.
  flatten_flag = kwargs.pop('flatten', False)
...
def filter_(predicate, *structures, **kwargs):
  ...
  def impl(predicate, *structures):
    ...
    if all(isinstance(s, (tuple, list)) for s in structures):
      ...
      if hasattr(structures[0], '_fields'):  # namedtuple
        filtered = (x if x != () else None for x in filtered)  # pylint: disable=g-explicit-bool-comparison
...
zip = zip_          # pylint: disable=redefined-builtin
map = map_          # pylint: disable=redefined-builtin
flatten = flatten_  # pylint: disable=redefined-builtin
filter = filter_    # pylint: disable=redefined-builtin
